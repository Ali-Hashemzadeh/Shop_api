...
3. **Cross-module communication only via Contracts + DTOs.** All interaction crosses the
   ...
   One addition: a module may also publish **integration events** in `Domain/Events/`
   carrying **primitives only** (ids, strings, numbers — never a model). These are public
   contract, like interfaces and DTOs; internal events stay private. Notification listens
   to them so business modules never depend on the Notification module.
...
- **Public codes (customer-facing identifiers).** Every entity a customer might quote to support carries a
  code shaped `<namespace><segment>-<SUFFIX>` (`bdp-K92XMQ`, `bdo-Q8M2XC`, …). Build them **only** through
  `App\Support\PublicCodeGenerator` + `App\Support\PublicCodeEntity` — never hard-code a prefix like `'bdp'`.
  Models opt in with `App\Support\HasPublicCode`, which checks uniqueness against **that model's own table
  only**. Namespace comes from the required `PUBLIC_CODE_NAMESPACE` env key; suffixes are six CSPRNG
  characters from `23456789ABCDEFGHJKMNPQRSTUVWXYZ` (never derived from an id, hash, timestamp, or sequence).
...
| Module | Status | Notes |
|---|---|---|
| **Identity** | ✅ Complete | OTP + password auth (split-auth onboarding), profiles, RBAC (`customer`/`admin`/`delivery`), admin user creation, provinces/cities, addresses |
...
- **A delivery worker is a shopper who also delivers: they hold `customer` *and* `delivery`.** The `delivery` role carries only
  `shipment.delivery.view-assigned` + `shipment.delivery.complete-assigned` and never duplicates the customer bundle. Adding or
  removing it is additive — never `syncRoles`. There is no `user` role; the shopper role is `customer`.
- **Admin surface:** `POST /api/v1/admin/users` (create a `customer` or `delivery` account — never an `admin`, never a password;
  `profile.create-any`, **plus** `profile.assign-delivery` for `role=delivery`), `POST /api/v1/admin/users/{user}/delivery-role`
  (idempotent, phone required, `profile.assign-delivery`), `GET /api/v1/admin/users?role=delivery`.
- **Shipment asks Identity, never the User model:** `IdentityManagerInterface::isDeliveryUser()`, `getDeliveryUserIds()`, and
  `getOwnedAddressSnapshot()` (→ `AddressSnapshotDTO`, which now carries `latitude`/`longitude`/`map_address` into the frozen
  shipment address snapshot).
- **Assignment belongs to the shipment, never the order,** and only to `local_delivery`. `POST /admin/shipments/{code}/assign-delivery`
  (`shipment.delivery.assign`). Reassignment closes the open `shipment_delivery_assignments` row and opens a new one in one
  transaction; assigning the current assignee is idempotent and notifies nobody.
- **A local delivery cannot enter `out_for_delivery` without an assignee** (422 on `assigned_delivery_user_id`), enforced in
  `ShipmentTransitionService` so both the first dispatch and the `delivery_failed` retry are covered. Admin still owns dispatch.
- **Dispatch mints a CSPRNG customer handoff code; only the hash is stored.** Never persist or expose the plaintext — not in the
  shipment, history, stored notifications, logs, or any API response. It rides on `ShipmentOutForDeliveryEvent` as a transient
  `deliveryCode` so local delivery still sends exactly **one** customer SMS (`shipment_out_for_delivery`).
- **A code is valid only for its attempt.** `delivery_failed` clears the hash; the next dispatch mints a new one; `delivered` consumes
  it. Reassignment mid-delivery does not regenerate it.
- **Completion needs the code from everyone.** Driver and admin routes share `MarkShipmentDeliveredAction`; only assignment
  authorization differs. There is no admin bypass. Non-assignee drivers get **404**, not 403 — the driver API is scoped structurally,
  never fetched-then-403'd.
- **Resend mints a new code (there is no old plaintext to recover), is SMS-only, and raises no second in-app notification.**
...
- **Product public identifier is a public code (stored in the `uuid` column).** `Product` carries a unique,
  server-generated `bdp-XXXXXX` code in the `uuid` column — **not** a v4 UUID (the column name is historical).
  Generated via `Product::generateUniquePublicCode()`; `generateUniqueUuid()` remains as a compatibility
  wrapper. Never accepted from client input. It is the API-facing handle: all product-level routes are
  `{uuid}`, constrained by `PublicCodeGenerator::routePattern(PublicCodeEntity::Product, '[0-9a-fA-F\-]+')`
  so **both** the new code and pre-existing 7-char hex codes (e.g. `a3f9c1b`) resolve while reserved segments
  like `admin`/`slug` stay excluded. Existing values were never rewritten. The `uuid` column is `string(16)`
  (not the native `uuid`/`char(36)` type, so short codes are valid on Postgres). Because seeders run under
  `WithoutModelEvents` (which mutes the `creating` hook) *and* `uuid` is not mass-assignable, the product
  seeder builds the model and assigns the code explicitly. The integer primary key stays internal and remains
  the FK target for variants/images. See "Public codes" below for the cross-cutting rules.
...
- Entities: `Category` (infinite nesting via `parent_id`, plus a `public_code` = `bdc-XXXXXX`), `Product`
  (`draft`/`published`, `uuid`, `primary_media_id`), `ProductImage` (gallery, `sort_order`), `ProductVariant`
