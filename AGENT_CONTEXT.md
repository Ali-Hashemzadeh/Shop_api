...
* **Responsibility:** Immutable financial contract anchor. Translates a validated cart into a locked order record, atomically reserves inventory, and manages a 15-minute pending-order TTL via a scheduled command.
  * **Tables:**
      ...
      * `order_items`: `id`, `order_id` (FK → orders, cascade delete), `sku`, `product_title`, `variant_attributes` (JSON), `product_snapshot` (nullable JSON — `{title, sku, image_url, primary_image_url, attributes}`; `image_url` is the variant image and `primary_image_url` the parent product primary image), `quantity` (int), `max_quantity_per_order_snapshot` (nullable int), `price_per_unit` (selling/base-price snapshot), `compare_at_price` (nullable display/reference snapshot), `line_total` (int), timestamps. Values come from the enriched `CartItemDTO`, never a second Catalog lookup. Compare-at price never participates in totals, tax, shipping, payments, or inventory. Existing rows/snapshots remain null/missing rather than being backfilled from current Catalog data.
  ...
  * **`CreateOrderAction`** — constructor deps: `CartManagerInterface`, `CatalogManagerInterface`, `InventoryManagerInterface`, `CancelOrderAction`, `ShipmentManagerInterface`, `IdentityManagerInterface`. Single `DB::transaction()`:
      ...
      5. Create each `OrderItem` from the enriched Cart DTO: `price_per_unit = basePrice`, nullable `compare_at_price = compareAtPrice`, and `product_snapshot` includes `imageUrl` (variant) plus `primaryImageUrl` (parent product). `line_total` remains the Cart-calculated selling-price total. **No second Catalog call**, then `reserveStock(sku, qty, orderId)`.
...
* **Responsibility:** Fulfillment lifecycle from checkout → payment → delivery/postal handoff/pickup. Owns the four fixed, **config-backed** methods, local-delivery working periods + generated dated slots + capacity/reservations, the operational shipment record, method-specific status workflows, and status history.
  ...
  * **Endpoints:** customer `GET /shipment/methods`, `GET /shipment/delivery-slots`, `GET /shipments/{publicCode}`, `GET /orders/{order}/shipment`; admin `GET /admin/shipments[/{publicCode}]` + business actions (`start-preparing`, `mark-ready-for-post`, `hand-to-post`, `mark-ready-for-dispatch`, `mark-out-for-delivery`, `mark-delivered`, `mark-delivery-failed`, `reschedule`, `mark-ready-for-pickup`, `confirm-pickup`) + slot mgmt (`GET/PATCH /admin/shipment/delivery-slots[/{slot}]`, `.../close`, `.../open`, `POST .../generate`). All under `throttle:api`. No generic "set status" endpoint. The literal `generate` route is declared **before** the `{slot}` routes so it is never read as a slot id.
...
|---|---|---|---|
| `OrderPaidEvent` (Order) | `EloquentOrderManager::markAsPaid`, after status + inventory commit + shipment activation, on the real transition only | `SendOrderPaidNotifications` | customer db+sms (`payment_success`), admin db (`admin_order_paid`) |
...
| `ShipmentSentEvent` | `ShipmentTransitionService` → `handed_to_post` / `out_for_delivery` | `SendShipmentSentNotifications` | customer db+sms (`OrderId` + `TrackingCode` when one exists) |
| `ShipmentDeliveredEvent` | `ShipmentTransitionService` → `delivered` | `SendShipmentDeliveredNotifications` | customer db+sms |
  ...
  * **Tests:** `tests/Feature/Notification/` (NotificationApiTest 9, NotificationDispatchTest 9, NotificationIntegrationTest 11) + `tests/Feature/Shipment/ShipmentNotificationTest` (5) — **34 tests**.
...
| Notification | ✅ Complete (events wired) | 34 passing across 4 test classes |
